          ...
          const contributorsWithNames = await Promise.all(
            contributorsData.map(async (contributor: Contributor) => {
              try {
                ...
                console.error(`Failed to fetch user data for ${contributor.login}:`, error);
              }
      ...
      <section className="py-12 bg-muted/50">
        <div className="container mx-auto px-4">
          <div className="text-center">
            ...
            <p className="mt-2 text-muted-foreground">Loading contributors...</p>
          </div>
        </div>
      </section>
    );
              ...
              <CardContent className="p-4">
                <div className="flex items-center space-x-3">
                  <Avatar className="h-12 w-12">
                    <AvatarImage src={contributor.avatar_url} alt={contributor.name || contributor.login} />
                    <AvatarFallback>
                      {(contributor.name || contributor.login).substring(0, 2).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
